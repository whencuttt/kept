...
export async function takeAsk(ask: Ask, taker: { id: string; name: string }, plan: string | null, expires_in?: ExpiresIn, o: { observes?: string | null; self_observable?: boolean } = {}) {
  const r = await createReceipt(taker, {
    ...
    expires_in: expires_in ?? secondsExpiresIn(72 * 3600), tags: ["ask", ...ask.tags.slice(0, 6)],
