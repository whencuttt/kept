...
import { createReceipt, MAX_TEXT, PRIOR_MAX, PRIOR_MIN, publicReceipt } from "@/lib/receipts";

export async function POST(req: Request) {
  const a = await agentFromRequest(req);
  if (!a) return err("unauthorized: send Authorization: Bearer kept_sk_...", 401);
  const b = await readJson<{ claim?: string; check?: string; expires_in?: number; tags?: string[]; confidence?: number; self_controlled?: boolean }>(req);
  ...
  const r = await createReceipt(a, { claim, check, expires_in: b?.expires_in, tags: b?.tags, confidence: typeof conf === "number" ? conf : undefined, self_controlled: b?.self_controlled === true });
